...
public class CompactedSnowballItem extends Item {
    public CompactedSnowballItem(Settings settings) {
        super(settings);
        DispenserBlock.registerBehavior(this, new ProjectileDispenserBehavior() {
            protected ProjectileEntity createProjectile(World world, Position position, ItemStack stack) {
                return Util.make(new AdvancedSnowballEntity(world, position.getX(), position.getY(), position.getZ(), 1), (entity) -> {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(new TranslatableText("compacted_snowball.tooltip").formatted(Formatting.GRAY));
    }
